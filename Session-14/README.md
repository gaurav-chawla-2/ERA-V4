# Session 14: DeepSeek-V3 Architecture Implementation

This project implements a **DeepSeek-V3** inspired language model from scratch, featuring **Multi-Head Latent Attention (MLA)** and a **Mixture of Experts (MoE)** architecture with loss-less load balancing.

## Architecture Overview

The model is built using the following advanced components:

### 1. Multi-Head Latent Attention (MLA)
To reduce Key-Value (KV) cache memory usage while maintaining performance, MLA projects keys and values into a low-rank latent space.
- **Hidden Size**: 576
- **Attention Heads**: 9
- **KV / Q LoRA Rank**: 128 (Compression factor)
- **RoPE**: Rotary Positional Embeddings applied to a subset of dimensions.

### 2. Mixture of Experts (MoE)
The Feed-Forward Network (FFN) is replaced by a MoE layer to increase total parameters without increasing compute cost (FLOPs) per token.
- **Total Experts**: 8 routed experts + 1 shared expert.
- **Active Experts**: Top-2 routed experts selected per token.
- **Expert Size**: Intermediate dimension of 512.
- **Shared Expert**: Always active to capture common knowledge, stabilizing training.

### 3. Loss-Less Load Balancing
Instead of using a complex auxiliary loss term to balance expert usage (which can hurt performance), we implement **Bias-Only Load Balancing**. 
- A bias term is added to the router logits.
- During training, if an expert is overloaded, its bias is decreased; if underloaded, it is increased.
- This dynamic adjustment ensures uniform utilization without gradients conflicting with the main objective.
...
The model was trained for 10,000 steps on the **TinyShakespeare** dataset. Below is the training log showing loss convergence and periodic text generation.
